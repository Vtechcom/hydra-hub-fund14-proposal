...
### 3.8. Nhật ký Sử dụng & Thống kê Giao dịch (Usage Logs & Transaction Statistics)

Đã tổng hợp nhật ký sử dụng và thống kê giao dịch bao gồm toàn bộ khung thời gian thử nghiệm 24 giờ.

**Nhật ký Trạng thái Nút Hydra:** [`state`](../state)

| Chỉ số | Giá trị |
|--------|--------|
| Tổng sự kiện ghi nhận | 8.148 |
| Khung thời gian log | `2026-03-10T11:37:33Z` → `2026-03-12T08:25:29Z` (~45 giờ) |
| Sự kiện đồng bộ chuỗi (`TickObserved`) | 5.802 |
| Sự kiện vòng đời deposit (`DepositExpired` / `DepositRecorded`) | 2.252 / 2 |
| Rollback chuỗi xử lý (`ChainRolledBack`) | 54 |
| Vòng đời Hydra Head | `HeadInitialized` → `HeadOpened` → `CommittedUTxO` (2) → `TransactionReceived` → `SnapshotConfirmed` → `HeadClosed` → `HeadFannedOut` |
| Đồng thuận snapshot (`PartySignedSnapshot`) | 3 |
| Kết nối mạng | `NetworkConnected` (1), `PeerConnected` (1) |

> Nhật ký trạng thái cung cấp bằng chứng có thể đọc bằng máy rằng nút đã hoạt động liên tục trong suốt giai đoạn thử nghiệm, xử lý thành công tất cả thao tác vòng đời Hydra, và xử lý ổn định các chain rollback.
